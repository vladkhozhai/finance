...
for (const char of invalidChars) {
  if (bearerToken.includes(char)) {
    console.log(`❌ Found invalid character: ${char.charCodeAt(0)} ('${char}')`);
    hasInvalidChar = true;
  }
}
...
if (bearerToken.includes(' ')) {
  console.log('⚠️  Contains SPACE character - this is allowed but can cause issues in some implementations');
}
